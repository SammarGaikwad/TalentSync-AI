...
3. **VerificationAgent (The Auditor):**
   ...
   - **Action:** Triggers automated tool tasks—including a **live integration with the public GitHub Search API** (fetching real candidate repositories, star counts, and language breakdowns in real time)—to verify claims against public coding profiles.
...
- **NVIDIA NIM (Neural Inference Microservices & Cloud API):**
  Llama-3 instruction completion is supported both **locally** (via local NIM container endpoints) and **in the cloud** (integrated with the official NVIDIA API Catalog via `NVIDIA_API_KEY`). The backend features an auto-router that dynamically intercepts local endpoints and redirects queries to `https://integrate.api.nvidia.com/v1/chat/completions` using the cloud model ID `meta/llama-3.1-70b-instruct` if a cloud token is present, running at sub-second responsiveness.
...
- **Multi-Service Vercel Deployment:** The repository is configured via [vercel.json](file:///d:/projects/TalentSync%20AI/vercel.json) to deploy as a unified multi-service app on Vercel, separating the Vite frontend and Express backend.
- **Stand-alone Server Fallback:** Express incorporates automatic `express.static` check routing in [server.js](file:///d:/projects/TalentSync%20AI/backend/server.js). If the production Vite build (`dist/`) exists, it hosts both frontend assets and backend APIs on the same port—enabling zero-config hosting on Render, AWS, and GCP.
